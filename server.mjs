...
const MODEL = "gemini-2.5-flash"; // Best for speed and medical reasoning
...
/* 🔥 MASTER MEDICAL PROMPT */
const MASTER_PROMPT = `
You are a highly experienced and globally trained medical doctor.
IMPORTANT RULES:
1. Understand any language, but respond in PROFESSIONAL MEDICAL ENGLISH.
...
app.post("/analyze-report", async (req, res) => {
  try {
    ...
    const ai = await callGemini("Analyze this medical report image.", doctor_pathy, image_base64);
    res.json(ai);
...
app.post("/analyze", async (req, res) => {
  try {
    ...
    let finalData;
    if (dbMeds.length >= 3) {
      finalData = { symptoms, medicines: dbMeds, diagnosis: "Clinical Record Match", diet: [], exercise: [], precautions: [] };
    } else {
    ...
    medicines.forEach(m => {
      ...
      if (source === "direct" || source === "edited") {
        batch.set(db.collection("medicine_master").doc(name), { 
          name, pathy: doctor_pathy, symptoms, usage_count: admin.firestore.FieldValue.increment(2), 
          verified: true, source: "doctor", updated_at: new Date() 
        }, { merge: true });
      }
    });
    await batch.commit();
...
app.listen(PORT, () => console.log(`🚀 ZEQVEX v1.5.1 LIVE ON PORT ${PORT}`));
